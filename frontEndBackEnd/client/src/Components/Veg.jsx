...
const Veg = () => {
    ...
    const deleteRecipie = (item) => {
        axios.delete(`http://localhost:7000/veg/${item.name}`)
        setVegData(prev => prev.filter(x => x.name !== item.name))
    ...
    const editRecipieDetails = (e) => {
        e.preventDefault()
        axios.put(`http://localhost:7000/veg/${id}`, details)
            .then(() => {
                setVegData(prev => prev.map(item => item.id === id ? details : item))
                setShow(false)
                    ...
                    return (
                        <Card style={{ width: '18rem' }}>
                            ...
                            <Card.Body>
                                ...
                                <Button variant="primary" onClick={() => editRecipie(x)}>Edit recipie</Button>
